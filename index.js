...
const totalMoney = GAMES_JSON.reduce((accumulator, game) => accumulator + game.pledged, 0);

// set inner HTML using template literal
raisedCard.textContent = `$${totalMoney.toLocaleString()}`;
...
function filterUnfundedOnly() {
    ...
    const unfundedGame = GAMES_JSON.filter((game) => game.pledged < game.goal);
...
function filterFundedOnly() {
    ...
    const fundedGame = GAMES_JSON.filter((game) => game.pledged >= game.goal);
...
const displayStr = `A total of $${totalAmountRaised.toLocaleString()} have been raised for ${GAMES_JSON.length} games. 
Currently, ${numberOfUnfundedGames} ${numberOfUnfundedGames > 1 ? 'games remain' : 'game remains'} unfunded. We need your help to fund these amazing games!`;
